...
*		the second parameter is optional, so it is the same as
*			if(timer1.done(500){...}
* 
* AUTO-REPEAT every 500ms, do this 10 times
* 		This uses the second parameter
* 		if(timer1.done(500, 10){...}
*
* SINGLE-SHOT after 500ms, do it once
*		This is actually AUTO-REPEAT with a value of one.
* 		if(timer1.done(500, 1){...}
*
* ENABLE
*		A timer can be started and stopped as required, and checked if it's running
* 		timer1.enabled(true);
...
*
* NEW feature in v2.2
* Dynamically change the 'done' timeout during a timing period e.g.
*			if(timer1.done(5000)){...};
*			if(timer1.done(200)){...}
